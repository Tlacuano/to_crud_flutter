...
def delete_barca(body):
    """Función para eliminar una barca de la base de datos."""
    conn = get_db_connection()

    try:
        with conn.cursor() as cur:
            sql = "DELETE FROM barcas WHERE id = %s"
            cur.execute(sql, (body['id'],))
            conn.commit()
            return {
                'statusCode': 200,
                'headers': open_headers,
                'body': json.dumps("Barca deleted successfully")
            }
    except Exception as e:
