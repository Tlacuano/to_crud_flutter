...
def update_barca(body):
    """Función para actualizar una barca en la base de datos."""
    conn = get_db_connection()

    try:
        with conn.cursor() as cur:
            sql = "UPDATE barcas SET nombre = %s, tipo = %s, longitud = %s, capacidad = %s WHERE id = %s"
            cur.execute(sql, (body['nombre'], body['tipo'], body['longitud'], body['capacidad'], body['id']))
            conn.commit()
            return {
                'statusCode': 200,
                'headers': open_headers,
                'body': json.dumps("Barca updated successfully")
            }
    except Exception as e:
